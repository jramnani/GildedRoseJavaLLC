    ...
    <>
      ...
      <div className="flex gap-x-[102px]">
        <table className="table-auto border-collapse text-left divide-y-2 divide-[#8690F4]">
          <thead>
            <tr className="flex-row space-x-52">
              <th className="py-3.5 pl-2 w-96 font-normal lowercase">
                Item Name
              </th>
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id} className="border-b-2 border-y-[#8690F4]">
        ...
        <div>
          <button
            onClick={updateItems}
            className="whitespace-nowrap py-[15px] px-[26px] w-40 lowercase bg-[#8690F4] rounded-[7px] text-[#222C40] font-medium"
          >
            Update Quality
          </button>
        </div>
